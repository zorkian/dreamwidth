...
| --- | --- |
| `update.bml` | Ordinary authenticated GET/POST, transforms, terminal and readonly GET slices are active. Authenticated share GET is active after independent public review and combined validation. Anonymous owner POST is active after the finite public HTTP and plain-app browser review; integration is correcting older retained-baseline test boundaries. Anonymous GET remains callable-only. Failed-auth/protocol outcomes stay native after an attempt, while structural exclusions retain BML. Alternate-login callable GET presentation and browser are accepted, and retained actor/session POST behavior is characterized. Native-form POST schema mapping and public alternate-login activation remain separate gates, alongside other excluded combinations. |
...
| `preview/entry.bml` | Legacy `js/entry.js` preview target, now served by `/preview/entry` non-BML routing (including `.bml` alias), preserving POST and localized GET/HEAD requirepost behavior. | **Removed** after preview parity; retain `htdocs/preview/entry.bml.text` for explicit legacy translation scope. |
...
| HTML helpers | `LJ::Web`, theme widgets, setting classes (`LJ::Widget::InboxFolder`/`InboxFolderNav` deleted with the inbox pages, 2026-09-23) | Remove legacy `entry_form`/`entry_form_decode` with their callers; retain shared helpers after making them independent of BML. Audit generated `<?...?>` blocks as well as named BML calls. |
