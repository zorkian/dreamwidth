...
13f4ea792, ten tracked `.bml` files remained: seven pages/endpoints and three
...
**Status (2026-09-23, after F2):** all seven pages/endpoints are graduated and
deleted (inbox's three, W1-W3; `update.bml`/`editjournal.bml`/`imgupload.bml`/
...
| `editjournal.bml` | **Deleted 2026-09-23 (F2).** Native `DW::Controller::EntryPicker::entry_picker_handler` owns `/editjournal` and the `.bml`-suffix-stripped `/editjournal.bml`: the no-item picker is unchanged, and the itemid branch always redirects (GET) or carries over through the native form (POST, T2). | **Graduated.** |
| `update.bml` | **Deleted 2026-09-23 (F2).** Native `DW::Controller::Entry`'s `/update` route (T2) owns both `/update` and the `.bml`-suffix-stripped `/update.bml`: GET always redirects, POST always carries over. | **Graduated.** |
| `preview/entry.bml` | **Deleted 2026-09-23 (F2).** Its only caller, `js/entry.js`, is deleted with it; `GET`/`POST /preview/entry` and `/preview/entry.bml` now 404. `_render_preview`'s legacy branch is removed; native `/entry/preview` is unchanged. | **Graduated.** |
| `tools/endpoints/draft.bml` | **Deleted 2026-09-23 (F2).** Its only caller, `js/entry.js`, is deleted with it; `/__rpc_draft` was already the native replacement. `GET /tools/endpoints/draft` now 404. | **Graduated.** |
| `imgupload.bml` | **Deleted 2026-09-23 (F2).** Its only caller, `js/entry.js`/InOb, is deleted with it; `GET /imgupload` now 404. `LJ::Widget::UserpicSelector`, whose only caller was `LJ::entry_form` (also deleted this package), is deleted with it. Its shared translation keys were already relocated to `/imguploadrte`'s own scope and `widget.userpicselector.link.view_thumbnails` in `bin/upgrading/en.dat` before this deletion; `imgupload.bml.text` is deleted with the page. | **Graduated.** |
...
| `inbox/index.bml` | **Deleted 2026-09-23 (W3).** Native `DW::Controller::Inbox::index_handler` owns `/inbox`, `/inbox/`, and the `.bml`-suffix-stripped `/inbox/index`. | **Graduated.** |
| `inbox/compose.bml` | **Deleted 2026-09-23 (W3).** Native `compose_handler` owns `/inbox/compose`. | **Graduated.** |
| `inbox/markspam.bml` | **Deleted 2026-09-23 (W3).** Native `markspam_handler` owns `/inbox/markspam`. | **Graduated.** |

### Companion files

The ten translation files (originally tracked) were the `.bml.text` companions of:

- `customize/index`, `customize/options`;
- `editjournal`, `update`, `preview/entry`, `imgupload` (deleted with their
  pages, 2026-09-23, F2; every key each file defined had already been
  relocated to its native home — `views/entry/form.tt.text`,
  `/entry/preview.tt`, `/imguploadrte`'s own scope, or the global
  `bin/upgrading/en.dat` — by W4/T2, and grep confirmed no code still read any
  `/update.bml.`, `/editjournal.bml.`, `/imgupload.bml.`, or
  `/preview/entry.bml.` key before the four files were deleted);
- `inbox/index`, `inbox/compose` (deleted with their pages, 2026-09-23; no
  `/inbox/*.bml.*` key was referenced anywhere else — verified by grep before
  deletion);
...
Modern manage templates use `/manage/circle/editfilters.bml.title2`. The FCK
image dialogs no longer use `/imgupload.bml.*` (relocated ahead of F2); the old
preview also references `/poll/create.bml.error.accttype2` even though that
page is migrated.
...
| HTML helpers | `LJ::Web`, theme widgets, setting classes (`LJ::Widget::InboxFolder`/`InboxFolderNav` deleted with the inbox pages, 2026-09-23; `LJ::Web::entry_form` and its entry_form-only helper `deemp`, and `LJ::Widget::UserpicSelector`, deleted with `update.bml`/`editjournal.bml`, 2026-09-23, F2) | `LJ::Web::entry_form_decode` is kept, delegating to `DW::Entry::Legacy::decode_entry_form` for the transitional carry-over handlers. Audit remaining generated `<?...?>` blocks as well as named BML calls in the rest of `LJ::Web`. |
