...
export function buildNativeReplay({
    corpus = defaultPreparedRoot,
    callsRoot = path.join(trackedCorpusRoot, 'native-calls'),
    output = path.join(corpus, 'native-derived-entry-cases.json'),
    mapOutput = path.join(corpus, 'native-call-replay-map.json'),
} = {}) {
    ...
    for (const suite of inventory.suites) {
        ...
        const tracePath = path.join(callsRoot, path.basename(suite.capturedCalls.path));
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
    buildNativeReplay({
        corpus: process.env.DW_CONTENT_CORPUS_ROOT || defaultPreparedRoot,
        output: process.argv[2] || path.join(process.env.DW_CONTENT_CORPUS_ROOT ||
            defaultPreparedRoot, 'native-derived-entry-cases.json'),
        mapOutput: process.argv[3] || path.join(process.env.DW_CONTENT_CORPUS_ROOT ||
            defaultPreparedRoot, 'native-call-replay-map.json'),
    });
}
