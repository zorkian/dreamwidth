...
export function snapshot(page: RawPageRequest["page"] = {kind: "recent", skip: 0, itemshow: 20}): RawJournalSnapshot {
    ...
    const data = {links: [],userpics: {pictures: [],mappings: []},owner, posters: [owner], fingerprint: "opaque-test-primary-generation-1",
...
export function selectFixture(
    data: Pick<RawJournalSnapshot, "owner" | "posters" | "style" | "entries" | "features" | "fingerprint"> & {userpics?: RawJournalSnapshot["userpics"];links?: RawJournalSnapshot["links"]},
    ...
    const common = {...data, links:data.links ?? [], userpics: data.userpics ?? {pictures: [],mappings: []}, request, calendar: {
