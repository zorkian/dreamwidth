// customtext-browser.ts
//
// Representative actual rich customtext with finite stock resource interception.
//
// Authors:
...
export async function customtextBrowser(html:string,port:number):Promise<void> {
    const output=process.env.S2_CUSTOMTEXT_BROWSER_OUTPUT;
    if(!output) return;
    const appOrigin="http://localhost:8080";
    const pageUrl=`http://localhost:${port}/users/ordinary6/76801.html`;
    assert.ok(process.env.S2_CUSTOMTEXT_STOCK_PAGE,"Set S2_CUSTOMTEXT_STOCK_PAGE from current entry-comparison.json");
    const prior=readFileSync(process.env.S2_CUSTOMTEXT_STOCK_PAGE,"utf8");
    ...
    try {
        ...
        const module=page.locator('.module-customtext');
        assert.equal(await module.count(),1);
        const state=await module.evaluate((element:HTMLElement)=>({text:element.innerText,
            color:getComputedStyle(element.querySelector('p')!).color,
            links:[...element.querySelectorAll('a')].map(anchor=>({href:anchor.href,label:anchor.textContent})),
            handlers:[...element.querySelectorAll('*')].flatMap(node=>node.getAttributeNames().filter(name=>name.startsWith('on')))}));
        assert.ok(state.text.includes('Custom rich'));
        assert.equal(state.color,'rgb(255, 0, 0)');
        assert.ok(state.links.some((link:any)=>link.href==='https://example.invalid/a?x=1&y=2'));
        assert.deepEqual(state.handlers,[]);
        mkdirSync(output,{recursive:true});
        await page.screenshot({path:path.join(output,"entry-customtext.png"),fullPage:true});
