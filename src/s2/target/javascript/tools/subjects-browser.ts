// subjects-browser.ts
//
// Representative actual rich subject/current formatting and safe destinations.
//
// Authors:
...
export async function subjectsBrowser(port:number, html:string):Promise<void> {
    const output=process.env.S2_SUBJECTS_BROWSER_OUTPUT;
    if(!output) return;
    const appOrigin="http://localhost:8080";
    const pageUrl=`http://localhost:${port}/users/ordinary6/76801.html`;
    assert.ok(process.env.S2_SUBJECTS_STOCK_PAGE,"Set S2_SUBJECTS_STOCK_PAGE from current entry-comparison.json");
    const prior=readFileSync(process.env.S2_SUBJECTS_STOCK_PAGE,"utf8");
    ...
    try {
        ...
        await context.route("**/*",async(route:any)=>{
            ...
            if(url==="https://subject.slice10.invalid/?x=1&y=2" && route.request().isNavigationRequest())
                return route.fulfill({status:200,contentType:"text/plain",body:"Intercepted declared subject destination"});
        ...
        const subject=page.locator(".entry-title");
        assert.ok((await subject.innerText()).includes("Rich café 😀"));
        const state=await subject.locator("b").evaluate((node:HTMLElement)=>{
            const style=getComputedStyle(node);return {color:style.color,weight:style.fontWeight,style:style.fontStyle};});
        assert.deepEqual(state,{color:"rgb(255, 0, 0)",weight:"700",style:"italic"});
        const mood=page.locator(".metadata-item-mood");
        assert.equal(await mood.innerText(),"custom mood");
        assert.equal(await mood.locator("i").getAttribute("style"),null);
        const location=page.locator(".metadata-item-location");
        assert.equal(await location.locator("cite").getAttribute("style"),null);
        assert.equal(await location.locator("a").getAttribute("href"),null);
        assert.equal(await location.locator("cite").evaluate((node:HTMLElement)=>getComputedStyle(node).position),"static");
        const link=subject.locator("a");
        assert.equal(await link.getAttribute("href"),"https://subject.slice10.invalid/?x=1&y=2");
        assert.equal(await page.locator('meta[property="og:title"]').getAttribute("content"),"Rich café 😀 subject link");
        assert.equal(await page.locator(".metadata-item-music").innerText(),"music & melody");
        assert.equal(await page.locator(".entry-title,.metadata").evaluateAll((nodes:Element[])=>
            nodes.some(node=>!!node.querySelector("script,iframe,object,embed,[onclick],[onerror]"))),false);
        mkdirSync(output,{recursive:true});
        await page.screenshot({path:path.join(output,"entry-subjects.png"),fullPage:true});
        await link.click();await page.waitForURL("https://subject.slice10.invalid/?x=1&y=2");
        ...
        writeFileSync(path.join(output,"browser.json"),JSON.stringify({pageUrl,fixtureStyle:44,
            ...
            stylesheetMapping:{emitted:expectedStyle[0],retained:retainedStyle[0]},computedSubject:state,
            subjectDestination:"https://subject.slice10.invalid/?x=1&y=2",
