// links-browser.ts
//
// Representative actual HTTP link destinations under captured stock assets.
//
// Authors:
...
export async function linksBrowser(port:number, html:string):Promise<void> {
    const output=process.env.S2_LINKS_BROWSER_OUTPUT;
    if(!output) return;
    const appOrigin="http://localhost:8080";
    const pageUrl=`http://localhost:${port}/users/ordinary6/76801.html`;
    assert.ok(process.env.S2_LINKS_STOCK_PAGE,"Set S2_LINKS_STOCK_PAGE from current entry-comparison.json");
    const prior=readFileSync(process.env.S2_LINKS_STOCK_PAGE,"utf8");
    ...
    try {
        ...
        assert.ok(links.some((link:any)=>link.href==="https://target.synthetic.invalid/?x=1&y=2" && link.title==="A 'quote' & tea"));
        assert.ok(links.some((link:any)=>link.href==="?literal=&amp;#39;"));
        assert.ok(links.some((link:any)=>link.href==="/website?q=1&x=2" && link.text==="Owner & website"));
        assert.deepEqual(failures,[]);
        mkdirSync(output,{recursive:true});
        await page.screenshot({path:path.join(output,"entry-links.png"),fullPage:true});
