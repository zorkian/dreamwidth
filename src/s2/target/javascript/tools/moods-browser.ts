// moods-browser.ts
//
// Representative actual selected mood icon with finite inert resource interception.
//
// Authors:
...
export async function moodsBrowser(html:string,port:number):Promise<void> {
    const output=process.env.S2_MOODS_BROWSER_OUTPUT;
    if(!output) return;
    const appOrigin="http://localhost:8080";
    const pageUrl=`http://localhost:${port}/users/ordinary6/76801.html`;
    assert.ok(process.env.S2_MOODS_STOCK_PAGE,"Set S2_MOODS_STOCK_PAGE from current entry-comparison.json");
    const prior=readFileSync(process.env.S2_MOODS_STOCK_PAGE,"utf8");
    ...
    for(let index=0;index<queue.length;index++) {
        ...
        if (url.href === "https://mood.slice11.invalid/pixel.png") {
            assets.set(url.href,{body:Buffer.from("iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAwMCAO+a3XcAAAAASUVORK5CYII=","base64"),
                type:"image/png",source:"declared inert synthetic mood icon"});continue;
        }
    ...
    try {
        ...
        const image=page.locator("#metadata-mood img");
        assert.equal(await image.getAttribute("src"),"https://mood.slice11.invalid/pixel.png");
        assert.equal(await image.getAttribute("width"),"16");
        assert.equal(await image.evaluate((node:HTMLImageElement)=>node.complete&&node.naturalWidth>0),true);
        assert.equal(await page.locator(".metadata-item-location").innerText(),"-0.0312,0.0312");
        const state=await image.evaluate((node:HTMLImageElement)=>({width:node.width,height:node.height,naturalWidth:node.naturalWidth}));
        mkdirSync(output,{recursive:true});
        await page.screenshot({path:path.join(output,"entry-moods.png"),fullPage:true});
        ...
        writeFileSync(path.join(output,"browser.json"),JSON.stringify({pageUrl,fixtureStyle:44,
            ...
            stylesheetMapping:{emitted:expectedStyle[0],retained:retainedStyle[0]},computedIcon:state,
