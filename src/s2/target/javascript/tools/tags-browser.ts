// tags-browser.ts
//
// Representative actual HTTP tag destinations under captured stock assets.
//
// Authors:
...
export async function tagsBrowser(port:number, html:string):Promise<void> {
    const output=process.env.S2_TAGS_BROWSER_OUTPUT;
    if(!output) return;
    const appOrigin="http://localhost:8080";
    const pageUrl=`http://localhost:${port}/users/ordinary6/76801.html`;
    assert.ok(process.env.S2_TAGS_STOCK_PAGE,"Set S2_TAGS_STOCK_PAGE from current entry-comparison.json");
    const prior=readFileSync(process.env.S2_TAGS_STOCK_PAGE,"utf8");
    ...
    // Exact image inputs created by the retained tag-nav script on interaction.
    for(const name of ["site/add","site/delete","entry/previous","entry/next"])
        queue.push(new URL("/img/silk/"+name+".png",pageUrl));
    ...
    try {
        ...
        await context.route("**/*",async(route:any)=>{
            ...
            if(url===pageUrl.slice(0,pageUrl.indexOf("/users/"))+"/go?dir=next&itemid=76801&journal=ordinary6&redir_key="+encodeURIComponent("public café & 😀") && route.request().isNavigationRequest())
                return route.fulfill({status:200,contentType:"text/plain",body:"Intercepted declared tag-nav destination"});
        ...
        const links=await page.locator('a').evaluateAll((nodes:HTMLAnchorElement[])=>
            nodes.map(node=>({href:node.getAttribute("href"),destination:node.href,title:node.title,text:node.textContent})));
        const expected="public café & 😀";
        assert.ok(links.some((link:any)=>link.text===expected && link.href.includes("caf%C3%A9")));
        assert.ok(!links.some((link:any)=>link.text.includes("NEVER_PROJECT_PRIVATE_TAXONOMY")));
        mkdirSync(output,{recursive:true});
        await page.screenshot({path:path.join(output,"entry-tags.png"),fullPage:true});
        await page.locator(".tag-nav-trigger").click();
        await page.locator(".tag ul a").filter({hasText:expected}).click();
        await page.locator(".tag-nav-actions input[data-dir=next]").click();
        await page.waitForURL((url:URL)=>url.pathname==="/go");
        assert.equal(page.url(),pageUrl.slice(0,pageUrl.indexOf("/users/"))+"/go?dir=next&itemid=76801&journal=ordinary6&redir_key="+encodeURIComponent(expected));
        ...
        writeFileSync(path.join(output,"browser.json"),JSON.stringify({pageUrl,fixtureStyle:44,
            ...
            stylesheetMapping:{emitted:expectedStyle[0],retained:retainedStyle[0]},links,
