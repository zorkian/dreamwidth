// userpic-browser.ts
//
// Representative actual HTTP userpic browser proof with declared fixture assets.
//
// Authors:
...
export async function userpicBrowser(port:number, html:string):Promise<void> {
    const output=process.env.S2_USERPIC_BROWSER_OUTPUT;
    if(!output) return;
    const appOrigin="http://localhost:8080";
    const pageUrl=`http://localhost:${port}/users/ordinary6/76801.html`;
    const prior=readFileSync(process.env.S2_USERPIC_STOCK_PAGE ||
        "artifacts/live/entry-oracle-384-one/page-oracle.html","utf8");
    ...
    for(const picid of [11,22]) assets.set(new URL(`/userpic/${picid}/900001`,pageUrl).href,
        {body:png,type:"image/png",source:"declared inert fixture pixel"});
    ...
    try {
        ...
        const pictures=await page.locator('img[src*="/userpic/"]').evaluateAll((nodes:HTMLImageElement[])=>
            nodes.map(node=>({src:node.getAttribute("src"),alt:node.alt,title:node.title,
                width:node.getAttribute("width"),height:node.getAttribute("height"),loaded:node.complete&&node.naturalWidth>0})));
        assert.ok(pictures.some((picture:any)=>picture.src==="/userpic/22/900001"&&picture.alt===
            "ordinary6: Selected description (chosen)"&&picture.title==="ordinary6: chosen (Selected description)"&&
            picture.width==="75"&&picture.height==="90"&&picture.loaded));
        assert.ok(pictures.some((picture:any)=>picture.src==="/userpic/11/900001"&&picture.loaded));
        assert.deepEqual(failures,[]);
        mkdirSync(output,{recursive:true});
        await page.screenshot({path:path.join(output,"entry-userpics.png"),fullPage:true});
        writeFileSync(path.join(output,"browser.json"),JSON.stringify({pageUrl,fixtureStyle:44,
            ...
            stylesheetMapping:{emitted:expectedStyle[0],retained:retainedStyle[0]},pictures,
