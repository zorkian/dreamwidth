...
test_psgi $app, sub {
    ...
    for my $index ( 0, 1 ) {
        ...
        my ($before_time) =
            $owner->selectrow_array( 'SELECT eventtime FROM log2 WHERE journalid=? AND jitemid=?',
        ...
        is( $form->value('itemid'),   $ditemid,  "$path retains the exact ditemid query contract" );
