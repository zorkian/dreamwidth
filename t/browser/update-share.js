// Browser acceptance for callable authenticated update share GET rendering.
...
const port = Number(process.env.UPDATE_SHARE_PORT || 18143);
const output = process.argv[2] || '/tmp/update-share-browser';
...
function waitPort() {
    return new Promise((resolve, reject) => {
        ...
        const connect = () => {
            ...
            socket.once('error', () => {
                socket.destroy();
                Date.now() > end ? reject(Error('share server did not listen')) : setTimeout(connect, 100);
            });
        };
        connect();
    });
...
(async () => {
    ...
    try {
        ...
        server = spawn(
            'perl',
            [process.env.LJHOME + '/t/browser/update-share-server.pl', String(port)],
        ...
        for (const width of [1280, 390]) {
            ...
            await page.goto(
                `http://127.0.0.1:${port}/__test_update_share?share=https%3A%2F%2Fexample.invalid%2Finput&prop_taglist=browser-tag&encoded=a%2Fb&repeated=one&repeated=two`,
        ...
        if (process.env.UPDATE_SHARE_INTENTIONAL_FAIL) {
            throw Error('intentional share cleanup failure');
        }

        assert.deepEqual(errors, []);
        assert.deepEqual(failures, []);
        console.log('PASS update share browser');
    }
    finally {
        try {
