...
(async () => {
    ...
    try {
        ...
        assert.equal(unsavedRuntime.settings, false, 'legacy jquery-group settings page does not load js/settings.js');
        assert.equal(unsavedRuntime.changed, undefined, 'no Settings.form_changed state exists to guard navigation');
