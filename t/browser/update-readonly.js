// Browser acceptance for callable retained readonly /update GET rendering.
...
const port = 18096;
const output = process.argv[2] || '/tmp/update-readonly-browser';
...
(async () => {
  ...
  try {
    ...
    server=spawn('perl',[process.env.LJHOME+'/t/browser/update-readonly-server.pl',String(port)],{stdio:['ignore','ignore','inherit']});
    ...
    if(process.env.UPDATE_READONLY_INTENTIONAL_FAIL) throw Error('intentional readonly cleanup failure'); assert.deepEqual(errors,[]);assert.deepEqual(failures,[]);console.log('PASS update readonly browser');
