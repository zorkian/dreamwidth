...
# This characterization intentionally owns the retained BML POST contract.
# Scope every method to an undef route result so DW::BML receives the original
# GET and POST, then assert the production public composition is restored.
sub with_retained_update_route {
    my ($callback) = @_;
    my $original = $DW::Routing::string_choices{'app/update'};
    die 'missing production update route' unless $original && $original->{sub};
    local $DW::Routing::string_choices{'app/update'} =
        { %$original, sub => sub { return undef; }, };
    ...
    sub {
        test_psgi $app, sub {
                ...
                )
            {
                ...
                is( $trace->{response}->code,
                    200, "valid password $path returns retained HTTP 200" );
                like(
                ...
                is_deeply( [ map { $_->{mode} } @{ $trace->{protocol} } ],
                    [qw(login postevent)],
                    "valid password $path runs one login then one postevent" );
                is_deeply(
                ...
                is( $trace->{protocol}[0]{has_pass},
                    0, "valid password $path login seed omits password" );
                is( $trace->{protocol}[1]{has_pass},
                    1, "valid password $path postevent seed has password" );
                is(
...
"valid password $path keeps update_fields GET data separate from the decoded POST request"
                ...
                is(
                    $after->{entries},
                    $before->{entries} + 1,
                    "valid password $path creates one entry"
                );
                is_deeply(
                ...
                )
            {
                ...
                if ( $case->{label} eq 'wrong password' ) {
                    ...
                    is_deeply( $trace->{protocol}, [],
                        "$case->{label} skips retained protocol requests" );
...

            {
                ...
                is( $trace->{response}->code,
                    200, 'forced protocol login failure retains HTTP 200' );
                like(
                    $trace->{response}->content,
                ...
                is( $trace->{scheduled}, 0,
                    'forced protocol login failure schedules no crossposts' );
