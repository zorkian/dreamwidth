...
#
# Worker for changing the poster ID of entries and comments.
#
# Authors:
...
sub work {
    ...
    unless ( $fu && $tu ) {
        $log->warn('Failed to load the involved users.');
    ...
    foreach my $cid (@LJ::CLUSTERS) {
        ...
        unless ($dbcm) {
            $log->warn("Temporary failure connecting to cluster $cid.");
            return DW::Task::FAILED;
        }

        eval {
            _fix_entries( $dbcm, $fu, $tu );
            _fix_comments( $dbcm, $fu, $tu );
        };
        if ($@) {
            $log->warn("Temporary failure fixing things: $@");
            return DW::Task::FAILED;
        }
    }
