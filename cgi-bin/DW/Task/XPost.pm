...
# DW::Task::XPost
#
# Worker for crossposting entries to external accounts.
#
# Authors:
#     Allen Petersen
#
...
sub queue_attributes {
    my ( $self, %opts ) = @_;
    my %attrs = $self->SUPER::queue_attributes(%opts);

    # Increase max receive count for crosspost retries
    if ( $opts{dlq} ) {
        my $arn =
...
sub work {
    ...
    unless ($u) {
        $log->warn("Unable to load user with uid $uid");
    ...
    unless ($acct) {
        $log->warn("Unable to load account $acctid for uid $uid");
        return DW::Task::FAILED;
    }
    ...
    # LJRossia is temporarily broken
...
"Crossposts to LJRossia are disabled until the remote site fixes their XMLRPC protocol handler.";
    ...
    # LiveJournal has broken client posts
    ...
    unless ( defined $entry && ( $delete || $entry->valid ) ) {
        $log->warn("Unable to load entry $ditemid for uid $uid");
        return DW::Task::FAILED;
    }
    ...
    if ( $result->{success} ) {
        ...
        $log->info(
            sprintf( "[xpost] Successful post to %s for %s(%d).", $domain, $u->user, $u->id ) );
    }
    else {
        # Connection timeout: retry
        if ( $result->{error} =~ /Failed to connect/ ) {
            $log->warn(
                sprintf( "[xpost] Timeout posting to %s for %s(%d)... will retry.",
                    $domain, $u->user, $u->id )
            );
            return DW::Task::FAILED;
        }
